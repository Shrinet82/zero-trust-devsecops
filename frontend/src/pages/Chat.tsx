...
export default function Chat() {
  ...
  const [messages, setMessages] = useState<
    { id: number; nickname: string; message: string }[]
  >([]);
  ...
  const addMessage = (nickname: string, message: string) => {
    setMessages((prevMessages) => [
      ...prevMessages,
      { id: nextId.current++, nickname, message },
    ]);
  };
  ...
  const userBancheck = (n: string): boolean => {
    ...
    return (
      banString !== null && JSON.stringify(JSON.parse(banString)).includes(n)
    );
  };
  const messagesComponent = messages
    .slice(isMobile ? -15 : -18)
    .map((message, index) => {
      return (
        <div
          key={index}
          ...
          style={
            index % 2 === 0 && index !== 0
              ? {
                  backgroundColor: settingThemeChat()["backgroundColor"],
                  display: userBancheck(message.nickname) ? "none" : "block",
                }
              : {
                  backgroundColor: settingThemeChat()["backgroundColor2"],
                  display: userBancheck(message.nickname) ? "none" : "block",
                }
          }
          ref={scrollDivRef}
        >
          <p
            className="message_paragraph"
            style={
              settingThemeChat()["backgroundColor"] === "#ffffff"
                ? { color: "black" }
                : {}
            }
          >
